...
import com.example.weatherapplication.utils.Converters.kelvinToCelsius
...
class HomeViewModel @Inject constructor(
    ...
    suspend private fun fetchForecast(coroutineScope: CoroutineScope) {
        homeRepository.getForecast("Bangalore").let { response ->
    ...
    suspend private fun fetchTodayWeather(coroutineScope: CoroutineScope) {
        homeRepository.getCurrentTemperature("Bangalore").let { response ->
            if (response.isSuccessful) {
                response.body()?.let { data ->
